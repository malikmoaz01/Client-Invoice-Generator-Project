...
const InvoiceGenerator = () => {
  ...
  const addEntry = () => {
    const mandatoryFields = ["sr", "date", "vehicle", "destination", "customerCode", "boxes", "weight", "rate", "extraStop"];
    for (const field of mandatoryFields) {
      if (!newEntry[field]) {
        alert(`The field "${field}" is mandatory and cannot be empty.`);
        return;
      }
    }

    const calculatedTotalAmount = parseFloat(newEntry.weight || 0) * parseFloat(newEntry.rate || 0);
    const isLahore = /^(lhr|lahore)$/i.test(newEntry.destination);
    const totalWithLahoreAdjustment = isLahore
      ? calculatedTotalAmount + (calculatedTotalAmount / 100) * 15
      : calculatedTotalAmount;

    const expenseAmount = totalWithLahoreAdjustment - parseFloat(newEntry.extraStop || 0);

    setEntries([
      ...entries,
      {
        ...newEntry,
        totalAmount: totalWithLahoreAdjustment.toFixed(2),
